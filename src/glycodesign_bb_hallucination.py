...
def halludesign_bb(
    ...
    set_seed()
    prot = basic_configs["name"]
    modify_chain_id = basic_configs["protein_chain_id"]
    sampled_sites = sample_sites(
        structure_file=wt_structure_file,
        scoring_df=f"{output_dir}/{prot}_single_points.csv",
        chain_id=modify_chain_id,
        num_sites_per_comb=num_gly_sites,
    )
    os.makedirs(os.path.join(output_dir, "design"), exist_ok=True)
    backbone_design(
        input_fasta_file=f"{output_dir}/{prot}.fasta", 
        output_dir=output_dir,
        modify_chain_id=modify_chain_id, 
        sampled_sites=sampled_sites,
    )
    mut, rmsd = backbone_filter(
        wt_pdb_file=wt_structure_file,
        output_dir=output_dir,
        chain_id=modify_chain_id,
    )
    ligandmpnn_redesign(
        input_fasta_file=f"{output_dir}/{prot}.fasta",
        backbone_fasta_file=f"{output_dir}/design/backbone.fasta",
        backbone_pdb_file=f"{output_dir}/design/backbones/{mut}",
        modify_chain_id=modify_chain_id,
        output_dir=output_dir,
        fixed_hotspots=basic_configs["functional_hotspots"],
    )